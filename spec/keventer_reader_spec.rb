...
describe KeventerReader do
  ...
  it 'Should Be Able to Load an Xml File for Events' do
    ...
    @connector.stub(:events_xml_url).and_return(File.join(File.dirname(__FILE__),
                                                          '../spec/events.xml'))
  ...
  it 'Should Be Able to Load an Xml File for an Event Type' do
    @connector = double('KeventerConnector')
    @connector.stub(:event_type_url).and_return(File.join(File.dirname(__FILE__),
                                                          '../spec/event_type_4.xml'))
    @connector.stub(:events_xml_url).and_return(File.join(File.dirname(__FILE__),
                                                          '../spec/events.xml'))
  ...
  it 'Should handle an empty eb-end-date as nil' do
    parser = LibXML::XML::Parser.file(File.join(File.dirname(__FILE__),
                                                '../spec/event_wo_eb_end_date.xml'))
    ev = event_from_parsed_xml(parser.parse)
    ev.eb_end_date.should be_nil
  end

  it 'Should handle an empty finish-date as nil' do
    parser = LibXML::XML::Parser.file(File.join(File.dirname(__FILE__),
                                                '../spec/event_wo_finish_date.xml'))
    ev = event_from_parsed_xml(parser.parse)
  ...
  context 'When loading the testing XML source with 18 events' do
    ...
    context 'when examining the second event' do
      before(:each) do
        @second_event = @kevr.events[1]
      end
  ...
  context 'Extracting kleerers' do
    before(:each) do
      @connector = double('KeventerConnector')
      @connector.stub(:kleerers_xml_url).and_return(File.join(File.dirname(__FILE__),
                                                              '../spec/kleerers.xml'))
